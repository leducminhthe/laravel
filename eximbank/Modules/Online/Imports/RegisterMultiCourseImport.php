...
class RegisterMultiCourseImport implements ToModel, WithStartRow
{
    ...
    public function model(array $row)
    {
        ...
        if($this->setStartRow >= 4){
            ...
            $course = OnlineCourse::whereCode($course_code)->first();
            ...
            if(isset($profile) && isset($course)){
                ...
                $course_object = OnlineObject::where('course_id', '=', $course->id)->pluck('title_id')->toArray();
            ...
            if(!$error) {
                ...
                if ($course->auto == 2){
                    ...
                    $quizs = Quiz::where('course_id', '=', $course->id)->where('status', '=', 1)->get();
                    if ($quizs){
                ...
                event(new SaveTrainingProcessRegister($course, $subject, $profile->user_id, null, 1));
            }
        }else{
            $this->setStartRow++;
        }
