...
class BC04 extends Model
{
    ...
    public static function getCourses($fromDate,$toDate,$training_from)
    {
        $fromDate = date_convert($fromDate);
        $toDate = date_convert($toDate,'23:59:59');
        $query = OfflineCourse::query()->where('status','=',1)
            ->where(function ($where) use ($fromDate,$toDate){
                $where->where('start_date','>=',$fromDate);
                $where->Where('end_date','<=',$toDate);
            });
        if ($training_from)
            $query= $query->where(\DB::raw('training_form_id'),'=',$training_from);
        $query= $query->select(\DB::raw("id,name as course_name,N'Tập trung' AS course_type, course_time,start_date,end_date,
                    training_unit,cost_class,null as teacher,null as students, null as attended,null as percent_attended,
                    null as unattended, null percent_unattended, null complete, null percent_complete, 
                    null uncomplete, null percent_uncomplete, null object"));
        return $query;
    }
    /*Xuất báo cáo*/
    public static function getData($fromDate,$toDate,$training_from)
    {
        $data = array();
        $courses = self::getCourses($fromDate,$toDate,$training_from)->get();
        foreach ($courses as $index => $item) {
            ...
            foreach (array_keys($item->original) as $i => $e) {
                ...
                elseif ($e=='teacher'){
                    ...
                    $row[$e]= OfflineObject::getObjects($item->id)[0]->object;
                }
                else
                    $row[$e]=$item->{$e};
            }
    ...
    public static function getTrainingFrom($training_form_id)
    {
        return TrainingForm::find($training_form_id);
    }
}
