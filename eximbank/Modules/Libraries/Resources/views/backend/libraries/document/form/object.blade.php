<form method="post" action="{{ route('module.libraries.document.save_object', ['id' => $model->id]) }}" class="form-horizontal form-ajax" role="form" enctype="multipart/form-data" data-success="submit_success">
    <div class="row">
        <div class="col-12 mb-3">
            <p style="color: red">{{ trans('lalibrary.note_object_document') }}</p>
        </div>
        <div class="col-md-9">
...
<div class="modal fade" id="modal-import" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <form action="{{ route('module.libraries.document.import_object', ['id' => $model->id]) }}" method="post" class="form-ajax">
...
<script type="text/javascript">
    var table = new LoadBootstrapTable({
        ...
        url: '{{ route('module.libraries.document.get_object', ['id' => $model->id]) }}',
        remove_url: '{{ route('module.libraries.document.remove_object', ['id' => $model->id]) }}'
    });

    var table_user = new LoadBootstrapTable({
        url: '{{ route('module.libraries.document.get_user_object', ['id' => $model->id]) }}',
        remove_url: '{{ route('module.libraries.document.remove_object', ['id' => $model->id]) }}',
