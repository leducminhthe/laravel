<form method="post" action="{{ route('module.saleskit.save_object', ['cate_id' => $categories->id, 'id' => $model->id]) }}" class="form-horizontal form-ajax" role="form" enctype="multipart/form-data" data-success="submit_success">
   <div class="row">
        <div class="col-12 mb-3">
            <p style="color: red">{{ trans('lalibrary.note_object_ebook') }}</p>
        </div>
        <div class="col-md-9">
            ...
            <div id="object-user">
                <div class="form-group row">
                    <div class="col-md-3"></div>
                    <div class="col-md-9">
                        <a class="btn" href="{{ download_template('mau_import_nhan_vien_thu_vien.xlsx') }}"><i class="fa fa-download"></i> {{trans('labutton.import_template')}}</a>
        ...
        <div class="col-md-12" id="form-object">
            <div id="table-object">
                <div class="text-right">
                    <button id="delete-unit-title" class="btn"><i class="fa fa-trash"></i> {{ trans('labutton.delete') }}</button>
                </div>
                <p></p>
                <table class="tDefault table table-hover bootstrap-table text-nowrap" id="table-unit-title-object">
                    <thead>
                        <tr>
...
<div class="modal fade" id="modal-import" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <form action="{{ route('module.saleskit.import_object', ['cate_id' => $categories->id, 'id' => $model->id]) }}" method="post" class="form-ajax">
...
<script type="text/javascript">
    var table = new LoadBootstrapTable({
        ...
        url: '{{ route('module.saleskit.get_object', ['cate_id' => $categories->id, $model->id]) }}',
        remove_url: '{{ route('module.saleskit.remove_object', ['cate_id' => $categories->id, $model->id]) }}',
        detete_button: '#delete-unit-title',
        table: '#table-unit-title-object',
    });

    var table_user = new LoadBootstrapTable({
        url: '{{ route('module.saleskit.get_user_object', ['cate_id' => $categories->id, $model->id]) }}',
        remove_url: '{{ route('module.saleskit.remove_object', ['cate_id' => $categories->id, $model->id]) }}',
