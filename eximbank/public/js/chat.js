$(document).ready(function () {
    Echo.private('chat.'+$('#user-id').val())
    .listen('MessageUser', (data)=>{
        console.log(data.socketId,Echo.socketId());
        if (data.type==1)
            adminSend(data.message.message);
        else {
            if (data.socketId != Echo.socketId())
                userSend(data.message.message);
        }
        // botProcess(data.message.message);
    })
...
function toogleChat($chatBot=0){
    if ($chatBot==1){
        $('.chat').toggleClass('is-visible').show();
        $('#chatuser').removeClass('is-visible').hide();
    }else{
        $('#chatuser').toggleClass('is-visible').show();
        $('.chat').removeClass('is-visible').hide();
    }
...
function toggleFab() {
    ...
    $('.fab').toggleClass('is-visible');
    // $('#chat_bot').toggleClass('is-visible');
    // $('.chat').toggleClass('is-visible');
    $
...
function saveMessage(text){
    try {
        const response = axios.post('/messages/user/', {
            message: text
        }).then( function () {
            axios.post('/messages/bot', {
                message: text
...
function botProcess(text){
        try {
            const response = axios.post('/messages/botProcess', {
                message: text
            })
        } catch (error) {
            console.log(error)
...
$(".friend").each(function(){
    $(this).click(function(){
        ...
        $(clone).css({'top': top}).addClass("floatingImg").appendTo("#chatuser");
        ...
        $('#chatuser #close').unbind("click").click(function(){
