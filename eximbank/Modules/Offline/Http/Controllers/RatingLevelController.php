...
class RatingLevelController extends Controller
{
    ...
    public function getData($course_id, Request $request){
        ...
        foreach ($rows as $row) {
            ...
            $row->modal_object_url = route('module.offline.rating_level.modal_add_object', ['course_id' => $course_id, 'id' => $row->id]);

            $object = OfflineRatingLevelObject::query()->where('course_id', '=', $course_id)->where('offline_rating_level_id', '=', $row->id);
            $row->modal_qr_code = '';
            if($object->exists()){
                $row->modal_qr_code = route('module.offline.rating_level.modal_qr_code', ['course_id' => $course_id, 'id' => $row->id]);
            }
        }
    ...
    public function save($course_id, Request $request){
        ...
        $course_edit = CourseTabEdit::firstOrNew(['course_id' => $course_id, 'course_type' => 2, 'tab_edit' => 'rating_level']);
        ...
        foreach ($categories as $category){
            ...
            foreach ($questions as $question){
                ...
                foreach ($answers_matrix as $answer_matrix){
                    ...
                    $new_answer_matrix->course_type = 2;
        ...
        json_result([
            ...
            'redirect' => route('module.offline.edit_rating_level', ['id' => $course_id])
        ]);
    }
    ...
    public function remove($course_id, Request $request){
        ...
        OfflineRatingLevel::whereIn('id', $ids)->where('course_id', '=', $course_id)->delete();
        OfflineRatingLevelObject::whereIn('offline_rating_level_id', $ids)->where('course_id', '=', $course_id)->delete();

        RatingTemplate2::whereIn('course_rating_level_id', $ids)->where('course_id', '=', $course_id)->where('course_type', 2)->delete();
        RatingCategory2::whereIn('course_rating_level_id', $ids)->where('course_id', '=', $course_id)->where('course_type', 2)->delete();
        RatingQuestion2::whereIn('course_rating_level_id', $ids)->where('course_id', '=', $course_id)->where('course_type', 2)->delete();
        RatingQuestionAnswer2::whereIn('course_rating_level_id', $ids)->where('course_id', '=', $course_id)->where('course_type', 2)->delete();
        RatingAnswerMatrix2::whereIn('course_rating_level_id', $ids)->where('course_id', '=', $course_id)->where('course_type', 2)->delete();
    ...
    public function modalQRCode($course_id, $id){
        $qrcode_rating_level = json_encode(['course'=>$course_id,'offline_rating_level_id'=>$id,'type'=>'rating_level_offline']);

        return view('offline::modal.qrcode_rating_level', [
    ...
    public function modalAddObject($course_id, $id){
        ...
        $offline_rating_level = OfflineRatingLevel::find($id);
        $profile = Profile::where('user_id', '>', 2)->get();

        $offline_rating_level_object = OfflineRatingLevelObject::query()
            ->where('course_id', '=', $course_id)
            ->where('offline_rating_level_id', '=', $id)
            ->get()->toArray();

        $result_object = [];
        foreach ($offline_rating_level_object as $object){
            $result_object[$object['object_type']] = $object;
        }

        return view('offline::modal.add_object_rating_level', [
            'course' => $course,
    ...
    public function getDataObject($course_id, $offline_rating_level_id, Request $request){
        $sort = $request->input('sort', 'id');
        $order = $request->input('order', 'desc');
        $offset = $request->input('offset', 0);
        $limit = $request->input('limit', 20);

        $query = OfflineRatingLevelObject::query()
            ->where('course_id', '=', $course_id)
    ...
    public function saveObject($course_id, $offline_rating_level_id, Request $request){
        $this->validateRequest([
            'object_type' => 'required',
            'object_rating' => 'required',
        ], $request, [
            'object_type' => 'Loại đối tượng',
            'object_rating' => 'Đối tượng được đánh giá',
        ]);

        $rating_level = OfflineRatingLevel::find($offline_rating_level_id);
        ...
        OfflineRatingLevel::query()
            ->where('id', '=', $offline_rating_level_id)
            ->update(['object_rating' => $object_rating]);

        OfflineRatingLevelObject::query()
            ->where('course_id', '=', $course_id)
            ->where('offline_rating_level_id', '=', $offline_rating_level_id)
            ->delete();
        ...
        foreach ($object_type as $type) {
            OfflineRatingLevelObject::query()
                ->updateOrCreate([
                    'course_id' => $course_id,
                    'offline_rating_level_id' => $offline_rating_level_id,
                    'object_type' => $type
                ],[
                    'course_id' => $course_id,
                    'offline_rating_level_id' => $offline_rating_level_id,
            ...
            if ($type == 1){
                $user_arr = OfflineRegister::whereCourseId($course_id)->whereStatus(1)->pluck('user_id')->toArray();
                foreach ($user_arr as $item){
                    $user = Profile::whereUserId($item)->first();
                    if ($user){
                        $signature = getMailSignature($item);
                        $params = [
                            'signature' => $signature,
    ...
    public function removeObject($course_id, $offline_rating_level_id, Request $request){
        $ids = $request->ids;

        OfflineRatingLevelObject::whereIn('id', $ids)
            ->where('course_id', '=', $course_id)
            ->where('offline_rating_level_id', '=', $offline_rating_level_id)
            ->delete();
    ...
    public function ajaxGetObject($course_id, $offline_rating_level_id, Request $request){
        $rating_level_object = OfflineRatingLevelObject::find($request->object_id);
    ...
    public function listReport($course_id,$class_id){
        $course = OfflineCourse::find($course_id);
        ...
        foreach ($anotherClass as $item) {
            $classArray[]=["name"=>$item->name,"url"=> route("module.offline.rating_level",['id'=>$course_id,'class_id'=>$item->id])];
        }
        ...
        return view('offline::backend.rating_level_result.list_report', [
            'course_id' => $course_id,
            'course' => $course,
    ...
    public function getdataListReport($course_id, Request $request) {
        ...
        foreach ($rows as $row) {
            ...
            $row->export = route('module.rating_level.report', [$course_id, 2, $row->id]);
        }
        json_result(['total' => $count, 'rows' => $rows]);
    }

    public function getdataListUserRating($course_id, $offline_rating_level_id, Request $request){
        $search = $request->input('search');
        ...
        $course = OfflineCourse::find($course_id);
        $offline_rating_level = OfflineRatingLevel::find($offline_rating_level_id);
        $offline_rating_level_object_4 = OfflineRatingLevelObject::query()
            ->where('offline_rating_level_id', '=', $offline_rating_level_id)
            ->where('course_id', '=', $course_id)
            ->where('object_type', '=', 4)
            ->first();
        if ($offline_rating_level_object_4){
            $user_arr = $offline_rating_level_object_4 ? explode(',', @$offline_rating_level_object_4->user_id) : [];
            $offline_register = OfflineRegister::whereCourseId($course_id)->whereStatus(1)->get();

            $start_date = $offline_rating_level_object_4->start_date ? $offline_rating_level_object_4->start_date : 'null';
            $end_date = $offline_rating_level_object_4->end_date ? $offline_rating_level_object_4->end_date : 'null';


            foreach ($offline_register as $register){
                $query = Profile::query()
                    ->select([
                        DB::raw(if_empty(@$offline_rating_level->object_rating, 'null') .' as object_rating'),
                        DB::raw(if_empty(@$offline_rating_level->level, 'null') .' as level'),
                        DB::raw(if_empty(@$offline_rating_level_object_4->id, 'null') .' as id'),
                        DB::raw(if_empty(@$offline_rating_level_object_4->object_type, 'null') .' as object_type'),
                        DB::raw(if_empty(@$offline_rating_level_object_4->time_type, 'null') .' as time_type'),
                        DB::raw(if_empty(@$offline_rating_level_object_4->num_date, 'null') .' as num_date'),
        ...
        $query->from('el_offline_rating_level as rating_level');
        $query->leftJoin('el_offline_rating_level_object AS object', 'object.offline_rating_level_id', '=', 'rating_level.id');
        $query->leftJoin('el_offline_register_view AS register', function ($sub){
        ...
        $query->leftJoin('el_offline_rating_level_object_colleague as object_colleague', function ($sub){
            $sub->on('object_colleague.offline_rating_level_id', '=', 'rating_level.id');
        ...
        $query->where('rating_level.id', '=', $offline_rating_level_id);
        ...
        foreach ($rows as $row) {
            ...
            if ($rating_status == 1){
                $row->export_word = route('module.rating_level.result.export_word', [$course_id, 1, $user_id, $offline_rating_level_id]);
            }
            if ($rating_status == 0){
                $row->result_url = route('module.offline.rating_level.modal_rating_level', [$course_id, $offline_rating_level_id, $user_id, $row->user_id]).'?rating_level_object_id='.$row->id;
            }else{
                $row->result_url = route('module.offline.rating_level.modal_edit_rating_level', [$course_id, $offline_rating_level_id, $user_id, $row->user_id]).'?rating_level_object_id='.$row->id;
            }
        }
        json_result(['total' => $count, 'rows' => $rows]);
    }
    ...
    public function modalRatingLevel($course_id, $course_rating_level, $user_id, $rating_user, Request $request) {
        ...
        return view('offline::modal.rating_level', [
            'item' => $item,
            'course_type' => 2,
            'template' => $template,
    ...
    public function modalEditRatingLevel($course_id, $course_rating_level, $user_id, $rating_user, Request $request) {
        ...
        return view('offline::modal.edit_rating_level', [
            'item' => $item,
            'course_type' => 2,
    ...
    public function saveRatingCourse($course_id, $course_rating_level_id, $user_id, $rating_user, Request $request){
        ...
        $redirect = route('module.offline.rating_level.list_report', [$course_id]);
    ...
    protected function updateMailUserRatingLevel(array $params, array $user_id, int $object_id){
        ...
        $automail->object_type = 'action_plan_reminder_offline_01';
