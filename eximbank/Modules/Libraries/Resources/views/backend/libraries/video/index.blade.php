...
@section('page_title', trans('lamenu.video'))
...
@section('content')
        ...
        <div class="row">
            <div class="col-md-12 form-inline">
                <form class="form-inline w-100 form-search mb-3" id="form-search">
                    <input type="text" name="search" class="form-control w-30" placeholder="-- {{ trans('lalibrary.video_author') }} --">
                    <div class="w-24">
            ...
            <div class="col-md-12 text-right act-btns">
                <div class="pull-right">
                    @can('libraries-video-edit')
                        <div class="btn-group">
                            <a href="{{ route('module.video.tree') }}" class="btn">
                    ...
                    @endcan
                    <div class="btn-group">
                        <a class="btn" href="{{ route('module.libraries.video.export') }}"><i class="fa fa-download"></i> {{ trans('labutton.export') }}</a>
                        @can('libraries-video-create')
                            <a href="{{ route('module.libraries.video.create') }}" class="btn"><i class="fa fa-plus-circle"></i> {{ trans('labutton.add_new') }}</a>
                        @endcan
                        @can('libraries-video-delete')
        ...
        <table class="tDefault table table-hover bootstrap-table">
            <thead>
                <tr>
                    ...
                    <th data-field="name" data-width="25%" data-formatter="name_formatter">{{ trans('backend.video_name') }}</th>
                    <th data-field="name_author" data-align="center" data-width="20%">{{ trans('lalibrary.author_name') }}</th>
                    <th data-field="category_parent_name">{{ trans('backend.video_category') }}</th>
    ...
    <script type="text/javascript">
        ...
        var table = new LoadBootstrapTable({
            ...
            url: '{{ route('module.libraries.video.getdata') }}',
            remove_url: '{{ route('module.libraries.video.remove') }}'
        });

        var ajax_isopen_publish = "{{ route('module.book.ajax_isopen_publish') }}";
