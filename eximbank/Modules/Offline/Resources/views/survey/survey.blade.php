                    ...
                    <form id="form_save_survey">
                        ...
                        <div class="certi_form mt-3">
                            <div class="all_ques_lest">
                                @php
                                    $categories = \Modules\Offline\Entities\OfflineSurveyCategory::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'template_id' => $template->id])->get();
                                @endphp
                                @foreach($categories as $cate_key => $category)
                                    ...
                                    @php
                                        $questions = \Modules\Offline\Entities\OfflineSurveyQuestion::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'category_id' => $category->id])->get();
                                    ...
                                    @foreach ($questions as $ques_key => $question)
                                        ...
                                        @php
                                            $answers = \Modules\Offline\Entities\OfflineSurveyAnswer::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'question_id' => $question->id])->get();
                                        @endphp
                                        ...
                                        <div class="ques_item mb-2">
                                            ...
                                            @elseif (in_array($question->type, ['matrix','matrix_text']))
                                                <div class="ui form survey ml-5">
                                                    <div class="grouped fields item-answer">
                                                        @php
                                                            $rows = \Modules\Offline\Entities\OfflineSurveyAnswer::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'question_id' => $question->id, 'is_row'=>1])->get();

                                                            $cols = \Modules\Offline\Entities\OfflineSurveyAnswer::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'question_id' => $question->id, 'is_row'=>0])->get();

                                                            $answer_row_col = \Modules\Offline\Entities\OfflineSurveyAnswer::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'question_id' => $question->id, 'is_row'=>10])->first();
                                                        @endphp
                                                        <table class="tDefault table table-bordered">
                                                            ...
                                                            @foreach($rows as $ans_row_key => $answer_row)
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    @foreach($cols as $ans_key => $answer_col)
                                                                        @php
                                                                            $matrix_anser_code = \Modules\Offline\Entities\OfflineSurveyAnswerMatrix::where(['course_id' => $course_id, 'course_activity_id' => $course_activity_id,'question_id' => $question->id, 'answer_row_id' => $answer_row->id,'answer_col_id'=> $answer_col->id])->first();
                                                                        @endphp
...
<script>
    ...
    $( document ).ready(function() {
        $('#send').on('click',function(event){
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('module.offline.survey.user.save') }}",
                dataType: 'json',
