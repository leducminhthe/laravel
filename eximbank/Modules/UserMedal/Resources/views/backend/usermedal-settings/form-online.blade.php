<p class="clearfix">
    @if(!$ro)
    <button id="onlinebtn" type="button" class="btn float-right" data-toggle="modal" data-target="#modal-online">
        <i class="fa fa-plus"></i> {{ trans('lamenu.add_online_course') }}
    </button>
    @endif
</p>
<div class="row">
	<div class="col-md-12">
		<table class="table border">
			...
			<tbody>
                @if($online_items)
                    @foreach($online_items as $index => $v)
                        <tr>
                            ...
                            <td class="border" style="text-align:center;">
                                @if(!$ro)
                                <a href="javascript:void(0)" class="text-primary edit-setting" data-type="online" data-id="{{ $v->id }}"><i class="fa fa-1x fa-edit"></i></a>
 ...
 <div class="modal fade" id="modal-online" aria-labelledby="ModalOnline" aria-hidden="true" data-backdrop="static" data-keyboard="false">
     <form action="{{ route('module.usermedal-setting.save-item',['form'=>'2','type'=>$model->id]) }}" method="post" class="form-ajax">
         <input type="hidden" name="settingitem_id" value="">
         <div class="modal-dialog" role="document">
             <div class="modal-content">
                 <div class="modal-header">
                     <h5 class="modal-title" id="ModalOnline">{{ trans('lamenu.online_course') }}</h5>
                 ...
                 <div class="modal-body">
                     ...
                     <div class="row">
                         ...
                         <div class="col-md-3">
                             <div class="form-group">
                                 <label>{{ trans('latraining.minute') }}</label>
                                 <select id="endminute" name="end_minute" class="form-control">
                                     @for($i=0;$i<=59;$i++)
                                     <option value="{{sprintf('%02d',$i)}}">{{sprintf('%02d',$i)}}</option>
                                     @endfor
                                 </select>
                     ...
                     <div class="row">
                         <div class="col-md-6">
                             ...
                             <select name="subject" id="subject_online" data-form="1" data-model="{{$model->id}}" class="form-control subject">
                         ...
                         <div class="col-md-6">
                             <label>{{ trans('latraining.course') }}</label>
                             <select class="form-control select2" name="item_id" id="sel_online" data-placeholder="-- {{ trans('lacategory.onl_course') }} --" >
                                 <option value="0">{{ trans('latraining.course') }}</option>
                             </select>
                         </div>
                     </div>
