...
use Modules\DashboardUnit\Entities\DashboardUnitPartByQuizType;
use Modules\Quiz\Entities\QuizPart;
use Modules\Quiz\Entities\QuizType;

class DashboardUnitPartByQuizTypeCommand extends Command
{
    protected $signature = 'dashboard_unit_part_by_quiz_type:update';
    protected $description = 'Cập nhật Thống kê số ca theo loại kỳ thi (Dashboard đơn vị). Chạy lúc 23h (0 23 * * *)';
    ...
    public function handle()
    {
        ...
        foreach ($units as $unit){
            ...
            foreach ($quiz_type as $item) {
                ...
                $model = DashboardUnitPartByQuizType::firstOrNew([
                    'unit_id' => $unit->id,
                ...
                for ($i = 1; $i <= 12; $i++) {
                    ...
                    $query->where(\DB::raw('month('.$prefix.'el_quiz_part.start_date)'), '<=', $ii)
                        ->where(function ($sub) use ($prefix, $ii){
                            $sub->orWhereNull('el_quiz_part.end_date');
                            $sub->orWhere(\DB::raw('month('.$prefix.'el_quiz_part.end_date)'), '>=', $ii);
                        });
            ...
            $query_sum = DashboardUnitPartByQuizType::where(['unit_id' => $unit->id,'year' => $year])
