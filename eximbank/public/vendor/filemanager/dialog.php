...
if (trim($rfm_subfolder) == "")
{
    ...
    $thumbs_path_absolute = $thumbs_base_path;
    $thumbs_path = $thumbs_base_path_relative;
    $parent 	 = $subdir;
}
else
{
    ...
    $thumbs_path_absolute = $thumbs_base_path. $rfm_subfolder;
    $thumbs_path = $thumbs_base_path_relative. $rfm_subfolder;
...
if (!is_dir($thumbs_path_absolute.$subdir))
{
    create_folder(FALSE, $thumbs_path_absolute.$subdir);
}
...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<!-- <script src="plugins.min.js?v=<?php echo time()?>"></script> -->
	    ...
	    <div class="tab-content">
			<div class="tab-pane active" id="tab1">
				...
				<form action="dialog.php" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
				    ...
				    <input type="hidden" name="path_thumb" value="<?php echo $thumbs_path_absolute.$subdir?>"/>
				    <div class="fallback">
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
		  $file=$file_array['file'];
			...
			if (!file_exists($thumbs_path_absolute.$subdir.$file)) create_folder(false,$thumbs_path_absolute.$subdir.$file);
			$class_ext = 3;
			if($file=='..' && trim($subdir) != '' ){
		    $src = explode("/",$subdir);
			...
			$file=$file_array['file'];
				...
				$src_thumb_absolute = $thumbs_path_absolute.$subdir. $file;
				//add in thumbs folder if not exist
				if(!file_exists($src_thumb_absolute)){
			    try {
			    	if(!create_img($file_path, $src_thumb_absolute, 122, 91)){
							$src_thumb=$mini_src="";
						}else{
            ...
            ?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<div class="img-precontainer">
				    ...
				    <div class="img-container">
					    <span></span>
					    <img class="aa <?php echo $show_original ? "original" : "" ?><?php echo $is_icon_thumb ? " icon" : "" ?><?php echo $lazy_loading_enabled ? " lazy-loaded" : ""?>" <?php echo $lazy_loading_enabled ? "data-original" : "src"?>="<?php echo $src_thumb; ?>">
				    </div>
				</div>
