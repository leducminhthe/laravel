<div id="myModal" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
        <form action="{{ route('module.offline.add-userpoint-setting-quiz', [$course->id]) }}" method="post" class="form-ajax">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control format_float_number" name="pvalue" autocomplete="off"  value="{{ $model->pvalue ? number_format($model->pvalue, 2) : '' }}" placeholder="{{ trans('latraining.enter_score_after_dot') }}">
                    </div>
                    <input type="hidden" name="item_id" value="{{ $course->id }}">
                    <input type="hidden" name="item_type" value="2">
                    <input type="hidden" name="pkey" value="offline_complete">
                </div>
    ...
    $('input[name=start_date]').on('change', function () {
        ...
        if(temp.length ==3){
            ...
            if(start_date < start_date_course || (end_date_course!='' && start_date > end_date_course)){
                ...
                $('.start_date_error').html('Từ ngày phải nằm trong khoảng thời gian khóa học');
            }else{
    ...
    $('input[name=end_date]').on('change', function () {
        ...
        if(temp.length ==3) {
            ...
            if (end_date < start_date_course || (end_date_course != '' && end_date > end_date_course)) {
                ...
                $('.end_date_error').html('Đến ngày phải nằm trong khoảng thời gian khóa học');
            } else {
                $('.end_date_error').html('');
            }
        }
