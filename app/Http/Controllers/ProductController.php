...
{   
    ...
    public function save_product(Request $request){
    	...
    	if ($get_image) {
    		//getClientOriginalName: lấy tển ảnh
    		$get_name_image = $get_image->getClientOriginalName();
    		//explode cách từ dấu chấm trả về chuổi
    		$name_image = current(explode('.', $get_name_image));
    		//getClientOriginalNameExtension đuôi mở rộng vd:jpg,png..
    		//rand(): trả về random số từ 0 - 99
    		$new_image = $name_image.rand(0,99).'.'.$get_image->getClientOriginalExtension();
    		$get_image->move('public/upload/product',$new_image);
    		$data['product_image'] = $new_image;
    		DB::table('tbl_product')->insert($data);
    		Session::put('message','Thêm sản phẩm thanh công');
    ...
    public function update_product(Request $request, $product_id){
    	...
    	if ($get_image) {
    		...
    		Session::put('message','Cập nhật sản phẩm thanh công');
    		return Redirect::to('all-product');
    	}

    	DB::table('tbl_product')->where('product_id', $product_id)->update($data);
    	Session::put('message','Cấp nhật sản phẩm thanh công');
    	return Redirect::to('all-product');
    }
    ...
    public function delete_product($product_id){
    	...
    	Session::put('message','Xóa sản phẩm thanh công');
    	return Redirect::to('all-product');
    }
    ...
    public function details_product($product_id){
        ...
        ->join('tbl_category_product','tbl_category_product.category_id','=','tbl_product.category_id')
        ->join('tbl_brand','tbl_brand.brand_id','=','tbl_product.brand_id')->where('tbl_product.category_id',$category_id)->whereNotIn('tbl_product.product_id',[$product_id])->get();
