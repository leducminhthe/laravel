...
class DoQuizController extends Controller
{
        ...
        foreach ($qids as $index => $question_id){
            ...
            if ($question['type'] == 'select_word_correct') {
                if ($anwsers) {
                    ...
                    $true_answer = array_intersect($anwsers,$question['correct_answers']);
    ...
    public function submitQuiz($quiz_id, $part_id, $attempt_id, Request $request) {
        ...
        if($quiz_by_online){
            $quiz = Quiz::find($quiz_id);
            $redirect = route('themes.mobile.frontend.online.detail', [$quiz->course_id]);
        }else{
        ...
        QuizAttempts::quizComplete($attempt_id,$quiz_id,$template['quiz'],$template['attemp']);
    ...
    public function saveFileQuestionEssay($quiz_id, $part_id, $attempt_id, Request $request){
        ...
        $template['questions'][$question_id]['link_file_essay'] = \link_download('quiz/' . $quiz_id . '/files/' . $filename);
        QuizAttempts::updateQuizData($attempt_id,$template);
        return response()->json([
            'status' => 'success',
            'message' => trans('laother.successful_save')
