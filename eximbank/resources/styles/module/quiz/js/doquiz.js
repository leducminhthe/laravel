$(document).ready(function() {
    ...
    var webcam_require = $('#webcam_require').val();
    ...
    function countDown(){
        var x = setInterval(function() {
            ...
            if (times_shooting_webcam){
                console.log(now.getTime(), (startTime.getTime() + times_shooting_webcam));
                if (now.getTime() >= (startTime.getTime() + times_shooting_webcam) || distance == 0 || (end_time && end_time == 0)){
                    ...
                    times_shooting_webcam += num_times_shooting_webcam;
                }
            }
            ...
            if (times_shooting_question){
                if (now.getTime() == (startTime.getTime() + times_shooting_question)){
                    ...
                    times_shooting_question += num_times_shooting_question;
                }
            }
        ...
        if(webcam_require == 1 && !disable) {
            ...
            if(navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                navigator.mediaDevices.getUserMedia(mediaConfig).then(function(stream) {
                    ...
                    $("#quiz-content .btn").prop('disabled', false);
                }).catch(error => {
                    var errorMessage = '';
                    if (error.name === 'NotAllowedError') {
                        errorMessage = $('#element_quiz').attr('data-not_allow_error')
                    } else if (error.name === 'NotFoundError') {
                        errorMessage = $('#element_quiz').attr('data-not_found_error')
                    } else if (error.name === 'NotSupportedError') {
                        errorMessage = $('#element_quiz').attr('data-not_suppor_error')
                    } else if (error.name === 'NotReadableError') {
                        errorMessage = $('#element_quiz').attr('data-not_readable_error')
                    } else if (error.name === 'OverconstrainedError') {
                        errorMessage = $('#element_quiz').attr('data-over_constrained_error')
                    } else {
                        errorMessage = $('#element_quiz').attr('data-qrcode_error_unknow')
                    }
                    $('.error_quiz').html(errorMessage);
                    $('#error_quiz').modal();

                    pageloadding(0);
                    $("#quiz-content .btn").prop('disabled', true);
                    checkCamera = 1;
                });
            }
    ...
    $('#form-submit .num-question-selected').html(numItems);
    ...
    function submitQuiz(form) {
        $.ajax({
            ...
            if (data.status == 'error' || data.text_quiz == 1){
                Swal.fire({
                    type: data.status,
                    html: data.message,
                    focusConfirm: false,
    ...
    function load_questions(page, scroll = null) {
        //var quizUrl = $('#element_app').attr('data-quiz_url');
            ...
            $.each(data.rows, function (i, item) {
                ...
                if(item.type == 'drag_drop_marker'){
                    ...
                    $.each(anwsers, function (i, a) {
                        ...
                        var style = drag_drop_marker_val ? 'style="min-width: 50px; min-height: 50px; position: absolute; left: '+left+'px; top: '+top+'px;"' : '';

                        answer_drag_marker += `<input type="hidden" name="q_${item.id}[${a.id}]" value="${drag_drop_marker_val}"/>`;
                        answer_drag_marker += '<span class="border mark_text" draggable="true" data-qid="'+item.id+'" data-id="'+a.id+'" '+style+'>';
                ...
                if(item.type == 'drag_drop_image'){
                    ...
                    $.each(anwsers, function (i, a) {
                        ...
                        if(a.marker_answer){
                            ...
                            var style = 'style="min-width: 50px; min-height: 50px; position: absolute; left: '+left+'px; top: '+top+'px;"';
                            ...
                            drop_image +=      '<span class="border drop_document" '+style+'>';
    ...
    function checkOutFullScreen() {
        var a = setInterval(function(){
            ...
            if((!window.screenTop && !window.screenY) || (window.screenTop < -207 && window.screenY < -207)){
                clearInterval(a);
                ...
                Swal.fire({
                    title: 'Thông báo',
                    text: "Bạn đã vi phạm quy chế Full screen khi thi. Bài thi sẽ bị khoá lại!",
                    icon: 'warning',
                    timer: 5000,
