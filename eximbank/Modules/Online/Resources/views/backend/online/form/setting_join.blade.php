...
<div class="row">
    <div class="col-md-9">
        @if($permission_save)
            <form method="post" action="{{ route('module.online.save_setting_join', ['id' => $model->id]) }}" class="form-ajax" id="form-setting-join" data-success="submit_success_setting_join">
...
<div class="modal fade" id="modal-import" tabindex="-1" role="dialog" aria-labelledby="modal-import" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document">
        <form action="{{ route('module.online.setting_join.import', ['id' => $model->id]) }}" method="post" class="form-ajax">
...
<script type="text/javascript">
    ...
    var table_setting_join = new LoadBootstrapTable({
        ...
        url: '{{ route('module.online.get_setting_join', ['id' => $model->id]) }}',
        remove_url: '{{ route('module.online.remove_setting_join', ['id' => $model->id]) }}',
    ...
    $('#table-setting-join').on('change', '.change-date-register', function(){
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('module.online.setting_join.change_date_register', ['id' => $model->id]) }}',
            dataType: 'json',
            data: {
    ...
    function runSettingJoin() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ route('module.online.run_cron_setting_join', ['id' => $model->id]) }}',
            dataType: 'json',
            data: {}
        }).done(function(data) {
            item.html(oldtext);
