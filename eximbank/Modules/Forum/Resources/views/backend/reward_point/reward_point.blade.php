<div class="row">
    ...
    <div class="col-md-4 text-right act-btns">
        <div class="pull-right">
            <div class="btn-group">
                <button type="button" onclick="saveScoreAnother(event)" class="btn save">{{ trans('labutton.save') }}</button>
            </div>
        </div>
    </div>
</div>
<br>
<table class="tDefault table table-hover bootstrap-table" id="table_another">
    <thead>
        <tr>
...
<script>
    ...
    var table_another = new LoadBootstrapTable({
        locale: '{{ \App::getLocale() }}',
        url: '{{ route('module.forum.reward_point.getdata', ['cate_id' => $cate_id, 'id' => $category->id]) }}',
        table: '#table_another',
    });
    ...
    function saveScoreAnother(event) {
        let item = $('.save');
        ...
        $.ajax({
            url: "{{ route('module.forum.reward_point.save', ['cate_id' => $cate_id, 'id' => $category->id]) }}",
            type: 'post',
            data: {
            ...
            if (data && data.status == 'success') {
                $(table_another.table).bootstrapTable('refresh');
            } else {
