...
class VideoController extends Controller
{
    ...
    {
        $categories = LibrariesCategory::select(['id','name'])->where('type', '=', 4)->get();
    ...
    public function treeFolder() {
        $name = 'video';
        $route = route('module.libraries.video');
        $corporations = LibrariesCategory::select(['id','name','type'])->where('type', '=', 4)->whereNull('parent_id')->get();
