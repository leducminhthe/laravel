<div>
    ...
    <span class="float-right">
        <button type="button" class="btn load-modal" data-url="{{ route('module.online.quiz.userpoint-setting-quiz', [$course_id, $model->id]) }}">
    ...
    <tbody>
        @foreach($userpoint as $v)
            <tr>
                ...
                <td class="text-center">
                    <i data-url="{{ route('module.online.quiz.edit-userpoint-setting-quiz', [$course_id, $model->id, $v->id]) }}" data-item="{{ $v->id }}" data-quiz="{{ $model->id }}" style="cursor: pointer;" class="far fa-edit load-modal"></i> 
...
<script>
    $('.table_setting_complete').on('click', '.remove-setting-item', function () {
        ...
        $.ajax({
            type: "POST",
            url: "/admin-cp/online/{{ $course_id }}/quiz/userpoint-setting/"+ quiz_id +"/delete/"+id,
            dataType: 'json',
            data: {
                'id': id,
            },
