function SCORMapi1_2(def, cmiobj, cmiint, cmistring256, cmistring4096, scormdebugging, scormauto, scormid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc) {
    console.log('aaa');
    var prerequrl = "/online/scorm/prereq";
    var datamodelurl = "/online/scorm/save";
    var datamodelurlparams = "id=" + cmid + "&a=" + scormid + "&sesskey=" + sesskey + "&attempt=" + attempt + "&scoid=" + scoid +"&_token=" + $('meta[name="csrf-token"]').attr('content');
    ...
    function initdatamodel(scoid) {
        prerequrl = cfgwwwroot + "/online/scorm/check-net?a=" + scormid + "&scoid=" + scoid + "&attempt=" + attempt + "&mode=" + viewmode + "&currentorg=" + currentorg + "&sesskey=" + sesskey;
        datamodelurlparams = "id=" + cmid + "&a=" + scormid + "&sesskey=" + sesskey + "&attempt=" + attempt + "&scoid=" + scoid +"&_token=" + $('meta[name="csrf-token"]').attr('content');
        cmi = new Object();
        cmi.core = new Object();
                        ...
                        if (matches != null) {
                            ...
                            if (errorCode == "0") {
                                if (autocommit && !(SCORMapi1_2.timeout)) {
                                    SCORMapi1_2.timeout = Y.later(60000, API, 'LMSCommit', [""], !1)
                                }
...
M.scorm_api = {};
M.scorm_api.init = function (Y, def, cmiobj, cmiint, cmistring256, cmistring4096, scormdebugging, scormauto, scormid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc) {
    window.API = new SCORMapi1_2(def, cmiobj, cmiint, cmistring256, cmistring4096, scormdebugging, scormauto, scormid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc)
};