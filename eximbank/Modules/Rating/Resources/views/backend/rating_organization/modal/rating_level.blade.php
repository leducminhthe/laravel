    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('module.rating_organization.save_rating_course', [$item->id, $rating_level->id, $user_id, $rating_user]) }}" method="post" class="form-validate form-ajax" role="form" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-12" id="custom-template">
                                    @php
                                        $categories = $template->category->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type);
                                    @endphp
                                    @foreach($categories as $cate_key => $category)
                                        ...
                                        @php
                                            $questions = $category->questions->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type);
                                            $num_ques = 1;
                                        @endphp
                                            ...
                                            <div class="ques_item mb-2">
                                                ...
                                                @elseif($question->type == 'dropdown')
                                                    <div class="ui form survey ml-5">
                                                        @php
                                                            $answers = $question->answers->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type);
                                                        @endphp
                                                ...
                                                @elseif (in_array($question->type, ['matrix','matrix_text']))
                                                    <div class="ui form survey ml-5">
                                                        <div class="grouped fields item-answer">
                                                            @php
                                                                $rows = $question->answers->where('is_row', '=', 1)->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type);

                                                                $cols = $question->answers->where('is_row', '=', 0)->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type);

                                                                $answer_row_col = $question->answers->where('is_row', '=', 10)->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type)->first();
                                                            @endphp
                                                            <table class="tDefault table table-bordered">
                                                                ...
                                                                @foreach($rows as $ans_row_key => $answer_row)
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        @foreach($cols as $ans_key => $answer_col)
                                                                            @php
                                                                                $matrix_anser_code = $question->answers_matrix->where('answer_row_id', '=', $answer_row->id)->where('answer_col_id', '=', $answer_col->id)->where('course_rating_level_id', $rating_level->id)->where('course_rating_level_object_id', $rating_level_object_id)->where('course_id', $item->id)->where('course_type', $course_type)->first();
                                                                            @endphp
