...
@section('breadcrumb')
    <div class="forum-container">
        @include('layouts.backend.menu_breadcum')
    </div>
@endsection
    ...
    <form method="post" action="{{ route('backend.category.unit.save', ['level' => $level]) }}" class="form-validate form-ajax " role="form" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="col-md-4 text-right">
                <div class="btn-group act-btns">
                    ...
                    @endcanany
                    <a href="{{ route('backend.category.unit', ['level' => $level]) }}" class="btn"><i class="fa fa-times-circle"></i> {{ trans('labutton.cancel') }}</a>
                </div>
            </div>
        </div>
        ...
        <div class="tPanel">
            <ul class="nav nav-pills mb-4" role="tablist" id="mTab">
                <li class="nav-item">
                    <a href="#base" class="nav-link active" role="tab" data-toggle="tab">{{ trans('latraining.info') }}</a>
                </li>
                <li class="nav-item">
