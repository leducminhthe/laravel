...
{{--            <input type="hidden" name="schedule" id="schedule" value="{{ $schedule_id }}">--}}
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{!! __('latraining.scan_qrcode_attendance') !!}</h5>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn" data-dismiss="modal">{!! __('labutton.close') !!}</button>
                </div>
            </div>
    </div>
</div>
    ...
    el: '#app',
            ...
            methods: {
                async onDecode (content) {
                    ...
                    var url = '{{url('qrcode/process') }}?user='+param.user_id+'&type=referer-course';
                    // window.location.href = url;
                    $.ajax({url:url, type:"get", dataType:"json",success: function (result) {
                        if(result.status=='success')
                            $('#referer').val(result.data);
