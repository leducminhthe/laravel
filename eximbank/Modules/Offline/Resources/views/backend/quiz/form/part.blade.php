<div role="main">
    <div class="row">
        <div class="col-md-12">
            <input type="hidden" class="form-control" name="id_part" value="">
...
<script type="text/javascript">
    ...
    function action_part_formatter(value, row, index) {
        return '<a href="javascript:void(0)" class="edit-item" data-id_part="'+row.id+'" data-name_part="'+ row.name +'" data-start_date="'+ row.startdate +'" data-end_date="'+ row.enddate +'" data-start_hour="'+ row.start_hour +'" data-start_min="'+ row.start_min +'" data-end_hour="'+ row.end_hour +'" data-end_min="'+ row.end_min +'"><i class="fa fa-edit text-info"></i></a>';
    }

    var table_part = new LoadBootstrapTable({
        locale: '{{ \App::getLocale() }}',
        url: '{{ route('module.offline.quiz.edit.getpart', ['course_id' => $course_id, 'id' => $model->id]) }}',
        table: '#table-part',
    });

    $('#table-part').on('click', '.edit-item',function () {
        var id_part = $(this).data('id_part');
        ...
        $('input[name=id_part]').val(id_part);
    ...
    $('.save-part').on('click', function() {
        ...
        var id_part = $('input[name=id_part]').val();
        ...
        $.ajax({
            url: '{{ route('module.offline.quiz.edit.savepart', ['course_id' => $course_id, 'id' => $model->id]) }}',
            type: 'post',
            data: {
                id_part: id_part,
                name_part: name_part,
