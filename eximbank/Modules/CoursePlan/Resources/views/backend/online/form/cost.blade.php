<form id="form" action="{{ route('module.course_plan.save_cost', ['course_type' => $course_type, 'id' => $model->id]) }}" method="post" class="form-ajax" data-success="form_cost">
    <div class="row">
        <div class="col-md-8">
            <label for="">{{trans('lamenu.unit')}}: </label><span>VNĐ</span>
        </div>
    ...
    <table class="tDefault table table-hover">
        ...
        <body>
            ...
            <tr>
                ...
                <th id="total_plan_amount">{{ number_format($total_plan_amount, 0) . ' VNĐ'}}</th>
                <th id="total_actual_amount">{{ number_format($total_actual_amount, 0) . ' VNĐ'}}</th>
                <th></th>
            </tr>
        </body>
    </table>
