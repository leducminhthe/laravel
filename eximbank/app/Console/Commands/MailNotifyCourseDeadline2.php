...
class MailNotifyCourseDeadline2 extends Command
{
    protected $signature = 'mail:notify_course_deadline_2';
    ...
    {
        $num_day = Config::getConfig('notify_course_deadline_2');
        if($num_day){
            ...
            foreach ($rows as $row) {
                ...
                foreach ($users as $user){
                    ...
                    $automail->template_code = 'notify_course_deadline_2';
                    $automail->params = [
                        'signature' => $signature,
                    ...
                    $automail->object_type = 'notify_course_deadline_2';
                    $automail->check_exists = true;
    
                    if ($automail->addToAutomail()) {
                        echo "mail:notify_course_deadline_2 ". $row->name ."\n";
                    }
                    else {
                        echo "mail:notify_course_deadline_2 not ". $row->name ."\n";
                    }
                }
            }
