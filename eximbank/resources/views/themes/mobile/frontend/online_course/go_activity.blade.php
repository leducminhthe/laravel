...
<div class="menu-activity bg-template" id="menu-activity">
    @foreach ($lessons_course as $key => $lesson_course)
        ...
        @foreach($activities as $activity)
            ...
            @if (($activity->activity_id != 2 && userThird()) || !userThird())
                <div class="row_activity row_activity_{{$activity->id}} p-2">
                    ...
                    @else
                        ...
                        @else
                            <span class="h6" id="activity_name_{{ $activity->id }}" onclick="activityCourse({{$item->id}},{{$activity->id}},{{$lesson_course->id}},{{$activity->activity_id}})">{{ $activity->name }}</span>
                        @endif
                    @endif
                </div>
            @endif
