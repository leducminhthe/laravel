...
@section('page_title', trans('lamenu.audio'))
...
@section('content')
        ...
        <div class="row">
            <div class="col-md-12 form-inline">
                <form class="form-inline w-100 form-search mb-3" id="form-search">
                    <input type="text" name="search" class="form-control w-30" placeholder="-- {{ trans('lalibrary.audiobook_author') }} --">
                    <div class="w-24">
            ...
            <div class="col-md-12 text-right act-btns">
                <div class="pull-right">
                    @can('libraries-audiobook-edit')
                        <div class="btn-group">
                            <a href="{{ route('module.audiobook.tree') }}" class="btn">
                    ...
                    @endcan
                    <div class="btn-group">
                        <a class="btn" href="{{ route('module.libraries.audiobook.export') }}"><i class="fa fa-download"></i> {{ trans('labutton.export') }}</a>
                        @can('libraries-audiobook-create')
                            <a href="{{ route('module.libraries.audiobook.create') }}" class="btn"><i class="fa fa-plus-circle"></i> {{ trans('labutton.add_new') }}</a>
                        @endcan
                        @can('libraries-audiobook-delete')
        ...
        <table class="tDefault table table-hover bootstrap-table">
            <thead>
                <tr>
                    ...
                    <th data-field="name" data-width="25%" data-formatter="name_formatter">{{ trans('lalibrary.audiobook') }}</th>
                    <th data-field="name_author" data-align="center" data-width="20%">{{ trans('lalibrary.author_name') }}</th>
                    <th data-field="category_parent_name">{{trans("backend.audiobook_category")}}</th>
    ...
    <script type="text/javascript">
        ...
        var table = new LoadBootstrapTable({
            ...
            url: '{{ route('module.libraries.audiobook.getdata') }}',
            remove_url: '{{ route('module.libraries.audiobook.remove') }}'
        });

        var ajax_isopen_publish = "{{ route('module.audiobook.ajax_isopen_publish') }}";
