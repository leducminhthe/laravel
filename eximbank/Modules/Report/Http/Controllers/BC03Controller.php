...
class BC03Controller extends ReportController
{
    public function review(Request $request, $key)
    {
        $report = parent::reportList();
        $year = $request->year;
        $param[]=array('name'=>'year','value'=>$year);
        $param[]=array('name'=>'report','value'=>$key);
    ...
    public function getData(Request $request)
    {
        ...
        $query = BC03::getCourses($fromDate,$toDate);
        $count = $query->count();
        ...
        foreach ($rows as $item) {
            ...
            foreach (array_keys($item->getOriginal()) as $i => $e) {
                ...
                elseif ($e == 'training_form_id'){
                    ...
                    $item->{$e} = $training_form ? $training_form->name : '';
                }
                elseif ($e == 'cost_class')
                ...
                elseif ($e == 'teacher'){
                    ...
                    $item->{$e} = OfflineObject::getObjects($item->id);
                }
                else
                    $item->{$e} = $item->{$e};
            }
