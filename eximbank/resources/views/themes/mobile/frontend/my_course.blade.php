...
@section('page_title', trans('app.my_course').' ('.$total.')')
                ...
                @foreach($my_course as $item)
                    @php
                        if ($item->course_type == 1){
                            ...
                            $route = route('themes.mobile.frontend.online.detail', ['course_id' => $item->course_id, 'my_course' => 1]);
                            $type = 'Online';
                            $image = image_online($item->image);
                            $go_activity_url = route('themes.mobile.frontend.online.detail.go_activity', ['course_id' => $item->course_id, 'my_course' => 1]);
                        }else{
                            ...
                            $route = route('themes.mobile.frontend.offline.detail', ['course_id' => $item->course_id, 'my_course' => 1]);
                            $type = trans('lamenu.offline_course');
                            $image = image_offline($item->image);
                            $go_activity_url = route('themes.mobile.frontend.offline.detail.go_activity', ['course_id' => $item->course_id, 'my_course' => 1]);
                        }
                    @endphp
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12 p-1">
                                        <a href="javascript:void(0);" onclick="loadSpinner('{{ $route }}', 1, 1)">
                                    ...
                                    <div class="col-12 p-1 align-self-center">
                                        ...
                                        <div class="text-right mb-2">
                                            <a href="javascript:void(0);" onclick="loadSpinner('{{ $go_activity_url }}', 1, 1)" class="btn bg-template link_btn">
                                                {{ trans('app.come_in_class') }}
                                            </a>
                                        </div>
