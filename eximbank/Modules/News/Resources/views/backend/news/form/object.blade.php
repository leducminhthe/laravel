<div class="row">
    <div class="col-12 mb-3">
        <p style="color: red">{{ trans('lanews.note_object_news') }}</p>
    </div>
    <div class="col-md-9">
    ...
    <form method="post" action="{{ route('module.news.save_object', ['id' => $model->id]) }}" class="form-horizontal form-ajax" role="form" enctype="multipart/form-data" data-success="submit_success">
        <input type="hidden" name="category_id" value="{{ $model->id }}">
        <div id="object-unit">
            <div class="form-group row">
                <div class="col-sm-3 control-label">
...
<div class="modal-dialog" role="document">
    <form action="{{ route('module.news.import_object', ['id' => $model->id]) }}" method="post" class="form-ajax">
...
var table = new LoadBootstrapTable({
    ...
    url: '{{ route('module.news.get_object', ['id' => $model->id]) }}',
    remove_url: '{{ route('module.news.remove_object', ['id' => $model->id]) }}'
});

var table_user = new LoadBootstrapTable({
    url: '{{ route('module.news.get_user_object', ['id' => $model->id]) }}',
    remove_url: '{{ route('module.news.remove_object', ['id' => $model->id]) }}',
