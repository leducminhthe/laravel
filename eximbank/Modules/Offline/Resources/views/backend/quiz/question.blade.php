...
<div class="main" id="quiz-question">
    <form method="post" action="" class="form-validate form-ajax" id="form-question" role="form" enctype="multipart/form-data">
        <div class="row">
            <div class="col-md-8">
                <a href="{{ route('module.quiz.question.review_quiz', ['id' => $quiz->id]) }}" class="btn">
                    <i class="fa fa-eye"></i> {{ trans('labutton.review_quiz') }}
                </a>
            </div>
            <div class="col-md-4 text-right">
                <div class="dropdown">
                    <button type="button" class="btn dropdown-toggle" {{ $disabled }} data-toggle="dropdown"><i class="fa fa-plus-circle"></i> {{trans('labutton.add_new')}} </button>
                    <div class="dropdown-menu">
                        <a href="javascript:void(0)" class="dropdown-item load-modal" data-url="{{ route('module.quiz.question.get_modal_quiz_question', ['id' => $quiz->id]) }}">{{trans('labutton.add_questionlib')}}</a>
                        <a href="javascript:void(0)" class="dropdown-item load-modal" data-url="{{ route('module.quiz.question.get_modal_question_category', ['id' => $quiz->id]) }}">{{trans('labutton.add_random_question')}}</a>
                    </div>
                </div>
                        ...
                        <li class="item-quiz-question category-question">
                            <div class="row">
                                ...
                                <div class="col-md-2 text-right">
                                    ...
                                    <a href="javascript:void(0)" class="edit-category {{ $disabled }}" data-category="{{ $qcat->id }}" data-num_order="{{ $quiz_question->num_order }}"><i class="fa fa-pencil"></i></a>
                                    <a href="javascript:void(0)" class="remove-category {{ $disabled }}" data-category="{{ $qcat->id }}"><i class="fa fa-trash text-danger"></i></a>
                                </div>
                            </div>
                        ...
                        <li class="item-quiz-question">
                            <div class="row">
                                <div class="col-md-10">
                                    ...
                                    <span class="custom-question">{!! $quiz_question->random ? trans('latraining.random').' ('.strip_tags(trim(html_entity_decode($category['name'], ENT_QUOTES, 'UTF-8'), "\xc2\xa0")).')' : strip_tags(trim(html_entity_decode($question['name'], ENT_QUOTES, 'UTF-8'), "\xc2\xa0"))  !!}</span>
                                    <input type="hidden" name="question[]" value="{{ $quiz_question->id }}">

                                </div>
                                <div class="col-md-2 text-right">
                                    <a href="javascript:void(0)" class="add-qqcategory {{ $disabled }}" data-num_order="{{ $quiz_question->num_order }}"><i class="fa fa-plus"></i></a>
                                    <input class="custom-score" {{ $disabled }} type="text" data-ques="{{ $quiz_question->id }}" value="{{ $quiz_question->max_score }}">
                                    <a href="javascript:void(0)" class="remove-question {{ $disabled }}" data-ques="{{ $quiz_question->id }}"><i class="fa fa-trash text-danger"></i></a>
                                </div>
                            </div>
                        </li>
                        ...
                        @foreach($qcats as $qcat)
                            <li class="item-quiz-question category-question">
                                <div class="row">
                                    ...
                                    <div class="col-md-2 text-right">
                                        <span><b>{{ $qcat->percent_group }} %</b></span>
                                        <a href="javascript:void(0)" class="edit-category {{ $disabled }}" data-category="{{ $qcat->id }}"><i class="fa fa-pencil"></i></a>
                                        <a href="javascript:void(0)" class="remove-category {{ $disabled }}" data-category="{{ $qcat->id }}"><i class="fa fa-trash text-danger"></i></a>
                                    </div>
                                </div>
                                <input type="hidden" name="question[]" value="c_{{ $qcat->id }}">
