...
<div class="row">
    <div class="col-md-9">
        <form method="post" action="{{ route('module.subject-type.save_object', ['id' => $model->id]) }}"
            class="form-horizontal form-ajax" role="form" enctype="multipart/form-data"
            data-success="submit_success">
            <div class="form-group row">
                <div class="col-sm-3 control-label">
                    <label>{{ trans('lasurvey.object_belong') }}</label>
                </div>
                <div class="col-md-6">
                    <label class="radio-inline"><input type="radio" name="object" value="1" checked>
                        {{ trans('lasurvey.unit') }} </label>
                    <label class="radio-inline"><input type="radio" name="object" value="2">
                        {{ trans('lasurvey.title') }} </label>
                    <label class="radio-inline"><input type="radio" name="object" value="3">
                        {{ trans('lasurvey.user') }} </label>
                </div>
            </div>
            <div id="object-unit">
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <div id="tree-unit" class="tree">
                            @foreach ($corporations as $item)
                                ...
                                @endphp
                                <div class="item">
                                    <i class="uil uil-plus"></i> <input type="checkbox" name="unit_id[]"
                                        data-id="{{ $item->id }}" class="check-unit" value="{{ $item->id }}">
                                    <a href="javascript:void(0)" data-id="{{ $item->id }}"
                                        class="tree-item">{{ $item->name . ' (' . $count_child . ') - ' . $item->profiles->count() }}
            ...
            <div id="object-title">
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        <select id="title_id" class="form-control select2"
                            data-placeholder="-- {{ trans('lasurvey.title') }} --" multiple>
            ...
            <div id="object-add">
                <div class="form-group row">
                    ...
                    <div class="col-md-9">
                        @can('subject-type-create-object')
                            <button type="submit" class="btn"><i class="fa fa-plus-circle"></i>
        ...
        <div id="object-user">
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <a class="btn"
                        href="{{ download_template('mau_import_nhan_vien_theo_username_chuong_trinh_dao_tao.xlsx') }}"><i
                            class="fa fa-download"></i> {{ trans('labutton.import_template') }}</a>
    ...
    <div class="col-md-12" id="form-object">
        <div id="table-object">
            <div class="text-right">
                @can('subject-type-delete-object')
                    <button id="delete-item-object" class="btn"><i class="fa fa-trash"></i>
                        {{ trans('labutton.delete') }}
                    </button>
                @endcan
            </div>
            <p></p>
        ...
        <div id="table-user-object">
            <div class="text-right">
                @can('subject-type-delete-object')
                    <button id="delete-user" class="btn"><i class="fa fa-trash"></i>
                        {{ trans('labutton.delete') }}
                    </button>
                @endcan
            </div>
...
<div class="modal fade" id="modal-import" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <form action="{{ route('module.subject-type.import_object', ['id' => $model->id]) }}" method="post"
            class="form-ajax">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn"
                        data-dismiss="modal">{{ trans('labutton.close') }}</button>
...
<script type="text/javascript">
    var table = new LoadBootstrapTable({
        ...
        url: '{{ route('module.subject-type.get_object', ['id' => $model->id]) }}',
        remove_url: '{{ route('module.subject-type.remove_object', ['id' => $model->id]) }}',
        detete_button: '#delete-item-object',
        table: '#table_object'
    });

    var table_user = new LoadBootstrapTable({
        url: '{{ route('module.subject-type.get_user_object', ['id' => $model->id]) }}',
        remove_url: '{{ route('module.subject-type.remove_object', ['id' => $model->id]) }}',
...
<script type="text/javascript">
    ...
    $('#title_id').on('change', function() {
        var title = $("#title_id option:selected").map(function() {
            return $(this).val();
        }).get();
        $('.title').val(title);
    });
        ...
        if ($(this).closest('.item').find('i:first').hasClass(openedClass)) {
            ...
            $.ajax({
                ...
                $.each(data.childs, function(i, item) {
                    ...
                    rhtml +=
                        '<i class="uil uil-plus"></i> <input type="checkbox" name="unit_id[]" class="check-unit" data-id="' +
                        item.id + '" value="' + item.id + '"> ';
                    rhtml += '<a href="javascript:void(0)" data-id="' + item.id +
                        '" class="tree-item"> ' + item.name + ' (' + data.count_child[item.id] +
                        ') </a>';
                    rhtml += '</div>';
    ...
    $('#tree-unit').on('click', '.check-unit', function(e) {
        ...
        if ($(this).is(":checked")) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('module.subject-type.get_child', ['id' => $model->id]) }}",
                dataType: 'json',
                data: {
                    id: id
                }
