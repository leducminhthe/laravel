...
<ul>
    ...
    <li>
        <div class="item">
            <i class="uil-minus uil"></i> <input type="checkbox" name="unit_id[]" class="check-unit" checked data-id="{{ $item->id }}" value="{{ $item->id }}">
            <a href="javascript:void(0)" data-id="{{ $item->id }}" class="tree-item">{{ $item->name .' ('. $count_child . ')' }}</a>
        </div>
        <div id="list{{ $item->id }}">
            @include('forum::backend.permission.tree_unit_child', ['parent_code' => $item->code])
        </div>
    </li>
    @endforeach
</ul>
