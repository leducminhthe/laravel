...
@section('breadcrumb')
    @php
        $breadcum= [
            ...
            [
                'name' => $quiz->name,
                'url' => route('module.quiz_template.edit', ['id' => $quiz->id])
            ],
            [
...
@section('content')
    <div class="main" id="quiz-question">
        <form method="post" action="" class="form-validate form-ajax" id="form-question" role="form"
            ...
            <div class="row">
                <div class="col-md-8">
                    <a href="{{ route('module.quiz_template.question.review_quiz', ['id' => $quiz->id]) }}" class="btn">
                        <i class="fa fa-eye"></i> {{ trans('labutton.review_quiz') }}</a>
                </div>
                <div class="col-md-4 text-right">
                    <div class="dropdown">
                        @can('quiz-template-question')
                            <button class="btn dropdown-toggle" type="button" data-toggle="dropdown"
                                    id="dropdownMenuQuestion" aria-haspopup="true" aria-expanded="false">
                                <i class="fa fa-plus-circle"></i> {{trans('labutton.add_new')}}
                            </button>
                        @endcan
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuQuestion"
                             style="right: 0px; left: unset;">
                            <a href="javascript:void(0)" class="dropdown-item load-modal"
                               data-url="{{ route('module.quiz_template.question.get_modal_quiz_template_question', ['id' => $quiz->id]) }}">{{trans('labutton.add_questionlib')}}</a>
                            <a href="javascript:void(0)" class="dropdown-item load-modal"
                               data-url="{{ route('module.quiz_template.question.get_modal_question_category', ['id' => $quiz->id]) }}">{{trans('labutton.add_random_question')}}</a>
                        </div>
                    </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-2 text-right">
                                                ...
                                                <a href="javascript:void(0)" class="edit-category"
                                                   ...
                                                   data-num_order="{{ $quiz_question->num_order }}"><i
                                                        class="fa fa-pencil"></i></a>
                                                <a href="javascript:void(0)" class="remove-category"
                                                   data-category="{{ $qcat->id }}"><i
                            ...
                            <li class="item-quiz-question">
                                <div class="row">
                                    <div class="col-md-10">
                                        ...
                                        <span class="custom-question">{!! $quiz_question->qcategory_id ? 'Ngẫu nhiên'.($quiz_question->difficulty ?  ' - ' .( $quiz_question->difficulty == 'D' ? 'Dễ' : ($quiz_question->difficulty == 'TB' ? 'Trung bình' : 'Khó' ))  : '').' ('.strip_tags(trim(html_entity_decode($category['name'], ENT_QUOTES, 'UTF-8'), "\xc2\xa0")).')' : strip_tags(trim(html_entity_decode($question['name'], ENT_QUOTES, 'UTF-8'), "\xc2\xa0"))  !!} </span>
                                        <input type="hidden" name="question[]" value="{{ $quiz_question->id }}">
                                    </div>
                                    <div class="col-md-2 text-right">
                                        <a href="javascript:void(0)" class="add-qqcategory"
                                           data-num_order="{{ $quiz_question->num_order }}"><i
                                                class="fa fa-plus"></i></a>
                                        <input class="custom-score" type="text" data-ques="{{ $quiz_question->id }}"
                                               value="{{ $quiz_question->max_score }}">
                                        <a href="javascript:void(0)" class="remove-question"
                                           data-ques="{{ $quiz_question->id }}"><i class="fa fa-trash text-danger"></i></a>
                                    </div>
                                </div>
                            </li>
                                ...
                                <li class="item-quiz-question category-question">
                                    <div class="row">
                                        ...
                                        <div class="col-md-2 text-right">
                                            <span><b>{{ $qcat->percent_group }} %</b></span>
                                            <a href="javascript:void(0)" class="edit-category"
                                               data-category="{{ $qcat->id }}"><i class="fa fa-pencil"></i></a>
                                            <a href="javascript:void(0)" class="remove-category"
                                               data-category="{{ $qcat->id }}"><i
                                                    class="fa fa-trash text-danger"></i></a>
                                        </div>
        ...
        <script type="text/javascript">
            $(function () {
                ...
                $(".remove-question").on('click', function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('module.quiz_template.question.remove_quiz_template_question', ['id' => $quiz->id]) }}",
                        data: {
                ...
                $(".custom-score").on('change', function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('module.quiz_template.question.update_max_score', ['id' => $quiz->id]) }}",
                        data: {
                            quiz_ques_id: quiz_ques_id,
                            max_score: max_score,
                        },
                ...
                $("#sortable").on('click', '.add-qqcategory', function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ route('module.quiz_template.question.modal_qqcategory', ['id' => $quiz->id]) }}',
                        dataType: 'html',
                        data: {
                            'num_order': num_order
                        }
                ...
                $("#sortable").on('click', '.remove-category', function () {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ route('module.quiz_template.question.remove_qqcategory', ['id' => $quiz->id]) }}',
                        dataType: 'html',
                        data: {
                            'num_order': num_order,
                            'category_id': category_id
                ...
                function update_question() {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '{{ route('module.quiz_template.question.update_num_order', ['id' => $quiz->id]) }}',
                        dataType: 'json',
            ...
            function editQuestionHandle(id, cateId) {
                var url = '<?php echo e(route("module.quiz.questionlib.question.edit", ["id" => ":id", "qid" => ":qid"])); ?>';
