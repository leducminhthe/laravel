...
class QuizQuestionController extends Controller
{
    ...
    //Modal đề mục trong danh sách câu hỏi
...
//            QuizQuestionCategory::updateCateTemplate($id);
...
//        QuizQuestionCategory::updateCateTemplate($id);
    ...
    public function saveQuestionRandom($id, Request $request) {
        ...
        if($random_question){
            ...
            $total_question_random = QuizQuestion::countQuestion($id, $cat_id); //Tổng số câu hỏi trong danh mục đã được thêm
            ...
            if($random_question_d){
                ...
                $total_question_random_d = QuizQuestion::countQuestion($id, $cat_id, 'D'); //Tổng số câu hỏi Dễ trong danh mục đã được thêm
            ...
            if($random_question_tb){
                ...
                $total_question_random_tb = QuizQuestion::countQuestion($id, $cat_id, 'TB'); //Tổng số câu hỏi TB trong danh mục đã được thêm
            ...
            if($random_question_k){
                ...
                $total_question_random_k = QuizQuestion::countQuestion($id, $cat_id, 'K'); //Tổng số câu hỏi Khó trong danh mục đã được thêm
...
//        QuizQuestionCategory::updateCateTemplate($quiz_id);
        json_message('ok');
    }
    ...
    public function updateMaxScore($quiz_id, Request $request){
        ...
        $quiz_question->save();
//        QuizQuestion::updateQuestionTemplate($quiz_id,$id);
//        QuizQuestionCategory::updateCateTemplate($quiz_id);
        json_message('ok');
    }
...
//        QuizQuestion::updateQuestionTemplate($quiz_id);
//        QuizQuestionCategory::updateCateTemplate($quiz_id);
        json_message('ok');
    }

    public function updateTemplateQuiz($quiz_id, Request $request){
        ...
        json_message('Đã cập nhật bộ đề thi');
    }
    ...
    protected function mapQuizQuestionCategories($quiz) {
        $qqcategorys = QuizQuestionCategory::whereQuizId($quiz->id)->orderBy('num_order', 'asc')->get();
    ...
    protected function mapQuizQuestions($quiz) {
        ...
        if ($quiz->shuffle_question == 1){
            $list_questions = QuizQuestion::with('question:id,name,type,multiple,multiple_full_score,answer_horizontal,image_drag_drop')
