...
function load_choose_unit(page, userUnit, type, loadMore) {
  var value_check_unit = $(".wrapped_checkbox_unit .unit_check_id").map(function () {
    return $(this).val();
  }).get();
...
function checkBoxUnitHandle(unitId) {
  ...
  if ($('#checkbox_unit_' + unitId).is(":checked")) {
    ...
    if ($('.wrapped_checkbox_unit').children().length == 0) {
      $('.wrapped_checkbox_unit').hide(); // $('.wrapped_checkbox_unit').html('<span class="title_checkbox ml-2">Chọn đơn vị</span>')
    }
  }
}
