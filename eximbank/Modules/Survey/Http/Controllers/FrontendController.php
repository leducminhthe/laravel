...
class FrontendController extends Controller
{
    ...
    public function getData(Request $request) {
        ...
        $query->whereNotNull('a.template_id');
       ...
       //dd($title_report, $content_report);
        ...
        if ($send == 1){
            ...
            if ($setting && $setting->point) {
                ...
                $user_point->user_id = $model->user_id;
    ...
    public function loadData($items) {
        ...
        foreach ($items as $item) {
            $survey = $item->users->where('id',profile()->user_id)->first();
            $format_endate = Carbon::parse($item->end_date)->format('Y-m-d');
            ...
            if ($survey && $survey->pivot->send == 1) {
                $data.='            <div class="badge_seller">'.trans('lasurvey.completed').'</div>';
            }
            $data.='            </div>';
            $data.='        </div>';
            ...
            $data.='                    <span class="vdt14"><b>'.trans('lasurvey.time').' : </b> <br>';
            $data.='                        <strong>'.Carbon::parse($item->start_date)->format('H:i d/m/Y') .'</strong>  '.trans('lasurvey.to').'';
            $data.='                        <strong>'.Carbon::parse($item->end_date)->format('H:i d/m/Y') .'</strong> ';
            $data.='                    </span>';
            $data.='                </div>';
            $data.='                <div>';
            if (!$survey && ($date < $format_endate)) {
            $data.='                    <a href="'.route('module.survey.user', ['id' => $item->id]).'" class="btn float-right">'.trans('lasurvey.take_survey').'</a>';
            } elseif (!$survey && ($date > $format_endate)) {
                $data.='                <button type="button" class="btn float-right">'.trans('lasurvey.end_survey').'</button>';
            } elseif ($survey && $survey->pivot->send == 1) {
                $data.='                <a href="'.route('module.survey.user.edit', ['id' => $item->id]).'" class="btn float-right">'.trans('lasurvey.view_survey').'</a>';
            } else {
                $data.='                <a href="'.route('module.survey.user.edit', ['id' => $item->id]).'" class="btn float-right">'.trans('lasurvey.edit_survey').'</a>';
            }
            $data.='                </div>';
            $data.='            </div>';
            $data.='        </div>';
