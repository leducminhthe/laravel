$(document).ready(function () {
    ...
    (function ($) {
        $.dragScroll = function (options) {
            ...
            var getCursor = function () {
                ...
                if (settings.scrollVertical && settings.scrollHorizontal) return 'url(themes/mobile/img/touch.png), move';
            ...
            $(document).on({
                ...
                'mouseup': function () {
                    clicked = false;
                    /*$('html').css('cursor', 'url(img/logo.png), auto');*/
                }
            });
        }
    }(jQuery))
    ...
    $('.theme-color .btn').on('click', function () {
        ...
        localStorage.setItem("theme_color_mobile", $(this).attr('data-theme'));
        $('html').addClass($(this).attr('data-theme'));

    });

    /* theme layout cookie */
    if ($.cookie("theme-color-layout") == 'dark') {
    ...
    $('#theme-dark').on('change', function () {
        if ($(this).is(':checked') === true) {
            ...
            $.cookie("theme-color-layout", 'dark', {
                expires: 1
            });
            ...
            $('html').removeClass($.cookie("theme-color-layout"));
            $.cookie("theme-color-layout", 'light', {
                expires: 1
            });
            $('html').addClass($.cookie("theme-color-layout"));
            nightModeMobile(0)
...
function nightModeMobile(type) {
    $.ajax({
        type: 'POST',
        url: "/AppM/night-mode-mobile",
        data: {
            'type' : type
        }
    }).done(function(data) {
