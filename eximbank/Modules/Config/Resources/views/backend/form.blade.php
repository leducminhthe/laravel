        ...
        <div class="card-body">
            <form action="{{ route('backend.config.email.save') }}" method="post" class="form-ajax">
                ...
                <div class="form-group">
                    ...
                    <select class="load-unit" data-level="0" name="company" data-placeholder="{{trans('lasetting.company')}}">
                        @if(isset($unit))
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="driver" placeholder="smtp" value="{{ $email->driver }}">
                </div>

                <div class="form-group">
                    <label>{{ trans('lasetting.email_host') }}</label>
                    <input type="text" class="form-control" name="host" placeholder="smtp.gmail.com" value="{{ $email->host }}">
                </div>

                <div class="form-group">
                    <label>{{ trans('lasetting.email_port') }}</label>
                    <input type="text" class="form-control" name="port" placeholder="587" value="{{ $email->port }}">
                </div>
                <div class="form-group">
                    <label>{{ trans('lasetting.send_from') }}</label>
                    <input type="text" class="form-control" name="from_name" value="{{ $email->from_name }}" placeholder="Hệ thống đào tạo">
                </div>
                <div class="form-group">
                    <label>{{ trans('lasetting.address_email_send') }}</label>
                    <input type="text" class="form-control" name="address" value="{{ $email->address }}" placeholder="hello@example.com">
                </div>
                <div class="form-group">
                    <label>{{ trans('lasetting.user_login') }}</label>
                    <input type="text" class="form-control" name="user" value="{{ $email->user }}">
                </div>
                <div class="form-group">
                    <label>{{ trans('lasetting.email_password') }}</label>
                    <input type="password" class="form-control" name="password" value="{{ $email->password }}">
                </div>
                <div class="form-group">
                    <label>{{ trans('lasetting.email_encryption') }}</label>
                    <input type="text" class="form-control" name="encryption" placeholder="tls" value="{{ $email->encryption }}">
                </div>
                <div class="form-group">
                    <input type="checkbox" id="send_noty" name="send_noty" value="1">
                    <label for="send_noty"> Gửi thông báo</label>
                </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('backend.config.email.test') }}" method="post" class="form-ajax">
                ...
                <input type="hidden" name="company" value="{{$email->company}}">
