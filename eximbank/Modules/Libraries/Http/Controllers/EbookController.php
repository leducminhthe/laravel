...
class EbookController extends Controller
{
    ...
    {
        $categories = LibrariesCategory::select(['id','name'])->where('type', '=', 2)->get();
        return view('libraries::backend.libraries.ebook.index', [
    ...
    public function getData(Request $request)
    {
        ...
        $query->where('el_libraries.type', '=', 2);
        if($search){
        ...
        foreach ($rows as $row) {
            ...
            $row->edit_url = route('module.libraries.ebook.edit', ['id' => $row->id]);
    ...
    public function form($id = 0) {
        ...
        $categories = LibrariesCategory::where('type','=',2)->get();
        ...
        if ($id) {
            ...
            return view('libraries::backend.libraries.ebook.form', [
                'model' => $model,
    ...
    public function save(Request $request) {
        ...
        if(!$request->attachment && !$request->post('path')){
        ...
        if ($model->save()) {
            ...
            if($request->post('path')){
                ...
                $lib_zip = LibrariesFileZip::firstOrNew(['libraries_id' => $model->id]);
                $lib_zip->libraries_id = $model->id;
                if($lib_zip->id){
                    if($lib_zip->origin_path != $path){
                        $lib_zip->status = 2;
                    }
                }
                $lib_zip->origin_path = $path;
                $lib_zip->save();
            }
            ...
            json_result([
                ...
                'redirect' => route('module.libraries.ebook')
            ]);
        }
    ...
    public function saveObject($libraries_id, Request $request){
        ...
        if ($parent_id && is_null($unit_id)){
            if (LibrariesObject::checkObjectUnit($libraries_id, $parent_id, 2)){
    ...
    public function importObject($libraries_id, Request $request){
        ...
        json_result([
            ...
            'redirect' => route('module.libraries.ebook.edit', ['id' => $libraries_id]),
        ]);
    }

    public function export()
    {
        return (new ExportLibraries(2))->download('danh_sach_ebook_'. date('d_m_Y') .'.xlsx');
    }
    ...
    public function treeFolder() {
        ...
        $route = route('module.libraries.ebook');
        $corporations = LibrariesCategory::select(['id','name','type'])->where('type', '=', 2)->whereNull('parent_id')->get();
