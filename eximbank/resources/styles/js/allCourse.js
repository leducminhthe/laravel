...
$('.search_course_type').on('click',function () {
    search_course_type = $("input[name=search_course_type]:checked").map(function(){return $(this).val();}).get();
...
$('.search_status').on('click',function () {
    search_status = $("input[name=status_course]:checked").map(function(){return $(this).val();}).get();
    ajaxSearchCourse(search_course_type, search_status, training_program_id, level_subject_id, fromdate, todate, search);
})

$('.training_program_checkbox').on('click',function () {
    training_program_id = $("input[name=training_program_id]:checked").map(function(){return $(this).val();}).get();
    ajaxSearchCourse(search_course_type, search_status, training_program_id, level_subject_id, fromdate, todate, search);
})

$('.level_subject_checkbox').on('click',function () {
    level_subject_id = $("input[name=level_subject_id]:checked").map(function(){return $(this).val();}).get();
    ajaxSearchCourse(search_course_type, search_status, training_program_id, level_subject_id, fromdate, todate, search);
})
...
if(course_type == 3) {
    ...
    function result_formatter_roadmap(value, row, index) {
        if (row.result == 1) {
    ...
    function detailTrainingByTitle() {
        $('#modal-training-by-title').modal();
    }

    //CHI TIẾT LỘ TRÌNH NGHỀ NGHIỆP
    function detailCareerRoadmap() {
                ...
                $.each(data.childs, function (i, item) {
                    if (type == 2){
                        rhtml_user += '<a href="javascript:void(0)" class="btn edit-roadmap-title mr-1" data-id="' + item.id + '">' +
                            '<i class="fa fa-edit"></i> ' + $('.element_data').attr("data-edit") +
                            '</a>' +
                            '<a href="javascript:void(0)" class="btn delete-roadmap-title mr-1" data-id="' + item.id + '">' +
                            '<i class="fa fa-trash"></i> ' + $('.element_data').attr("data-delete") +
                            '</a>';
                    }
...
function horizontalMenu(course_type) {
    let url = $('.element_data').attr("data-url_all_course") + '?list=horizontal';
    window.location.href = url;
}

// DANH SÁCH NẰM DỌC
function verticalMenu(course_type) {
    let url = $('.element_data').attr("data-url_all_course") + '?list=vertical';
    window.location.href = url;
}